...
1. [こちらの定義ファイル](via_definition/suika15tone.json) をダウンロードし、適当な場所に保存する
2. Chrome または Edge ブラウザで https://remap-keys.app/ にアクセスする
3. 「キーボードをカスタマイズ」ボタンをクリックする
4. 「+ キーボード」をクリックすると、「remap-keys.appがHIDデバイスへの接続を要求しています」という画面が開くので「suika15tone」を一覧から選択し「接続」をクリックする
5. 「インポート（.json）」ボタンをクリックし、1.でダウンロードしたファイルを選択
8. 下図のようにキーボードのデザインが正しく表示されていれば、キーマップの編集準備完了です  
      ![remap](img/remap.png)
